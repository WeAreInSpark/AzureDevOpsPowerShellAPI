BeforeAll {
  ...
  $path = Join-Path -Path $PSScriptRoot -ChildPath "..\$ModuleName\$ModuleName.psd1" | Resolve-Path
  $manifest = Import-PowerShellDataFile -Path $path
  $manifestData = Test-ModuleManifest -Path $path -Verbose:$false -ErrorAction Stop -WarningAction SilentlyContinue
